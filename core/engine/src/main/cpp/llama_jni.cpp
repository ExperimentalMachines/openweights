...
Java_io_github_alpharomercoma_openweights_core_engine_LlamaBridge_nativeLoadModel(
    ...
    Session * session = Session::load(
        ...
        kv_quantized == JNI_TRUE, speculate == JNI_TRUE, image_tokens,
        batch_tokens, micro_batch_tokens, error);
    if (session == nullptr) {
...
Java_io_github_alpharomercoma_openweights_core_engine_LlamaBridge_nativeWarm(
    ...
    const std::string store =
        store_path == nullptr ? std::string() : to_utf8(env, store_path);

    WarmStats stats;
    std::string error;
    const bool ok = session->warm(
        messages, tools, thinking, snapshot == JNI_TRUE,
        store.empty() ? nullptr : store.c_str(), stats, error);
